...
def send_ntfy_notification(title, message, priority="default", tags=None):
    ...
    try:
        ...
        # Catches network errors AND any header-encoding issues, so a notification
        # problem can never crash the whole uploader script.
        print(f"Error while sending ntfy notification: {e}")
...
def handle_alert_transition(data):
    ...
    if new_state == "alarm":
        ...
        send_ntfy_notification(
            title=f"Smoke Alert - Sender {sender_id}",
